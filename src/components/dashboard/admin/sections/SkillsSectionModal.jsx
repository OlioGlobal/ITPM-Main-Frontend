...
export default function SkillsSectionModal({
  isOpen,
  onClose,
  pageId,
  section,
  ...
  useEffect(() => {
    if (section?.data) {
      ...
      setValue("skills", section.data.skills?.map((s) => ({ value: s })) || []);
    }
  }, [section, setValue]);
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      if (section) {
        ...
        toast.success("Skills section created");
      }
      ...
      <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white">
          <h2 className="text-xl font-bold text-gray-900">
            {section ? "Edit" : "Add"} Skills Section
          </h2>
          <button
            onClick={onClose}
